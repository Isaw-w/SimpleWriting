...
final class GrammarClient {
    ...
    Also flag a short span that genuinely does not make sense as written — garbled word order, a missing word that breaks the meaning, or a fragment that cannot be parsed — but only when it truly does not make sense, not when it is merely unusual, informal, or stylistically imperfect yet still understandable.
